...
//Rare exceptions to indicate a bug in the _bookbindingex code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_bookbindingex = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_bookbindingex]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.bookbindingex.module"
